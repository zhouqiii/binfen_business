    ...
    <el-container >
      <el-main>
        ...
        >
          ...
          <el-row type="flex" justify="start" align="middle">
            <el-col :span="15">
              <el-form-item label="订单名称:" prop="orderName" >
                <el-select v-model="ruleForm.orderName" disabled filterable placeholder="请选择或直接输入名称"  style="width: 100%" >
          ...
          <el-row type="flex" justify="start" align="middle">
            <el-col :span="15">
              <el-form-item label="细化名称:" prop="refineName" style="margin-top: 0px;margin-bottom: 0px;">
                <el-checkbox-group v-model="refineName" >
                  <el-checkbox v-for="item in projectRefines " @change="totalWorkload(item,$event)"
          ...
          <el-row type="flex" justify="start" align="middle">
            <el-col :span="15">
              <el-form-item label="订单状态:" prop="orderState" >
                <el-select v-model="ruleForm.orderState" filterable placeholder="请选择" style="width: 100%">
                  <el-option
...
<script>
import {getOrderStatus, getProjectNames,selectRefinesByProjectId,editOrder} from "@/api/businessOrder";
...
export default {
  ...
  methods: {
    init(){
      ...
      this.selectRefinesByProjectId(this.ruleForm.projectId);
      this.ruleForm.orderState = this.ruleForm.msg;
      this.refineName = this.ruleForm.refineName.split(",");
      this.ruleForm.refineName = this.ruleForm.refineName.replace(/,/g,"-")+ "-";
    ...
    totalWorkload(item,event){
      ...
      //新建订单所对应的细化id集
      let id = item.refineName + "-"
      if(event){
        this.ruleForm.refineName += item.refineName + "-"
      }else {
    ...
    submitForm(formName) {
      ...
      let refineIds = "";
      let refineNames = this.ruleForm.refineName.split("-");
      for (let i = 0; i < refineNames.length; i++) {
        for (let j = 0; j < this.projectRefines.length; j++) {
          if (refineNames[i] == this.projectRefines[j].refineName) {
            refineIds += this.projectRefines[j].refineId + "-";
          }
        }
      }
      this.ruleForm.refineName = refineIds;
