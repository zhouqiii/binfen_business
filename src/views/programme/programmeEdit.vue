...
<script>
  ...
  import { infoProgramme,editProgramme,infoByProgrammeId,checkChildDemandOnUpdate } from "@/api/businessProgramme";
  import { projectList,refineListById,refineInfoById } from "@/api/businessProject";
  import { upload,getListById } from "@/api/businessZipFile";
  ...
  export default {
    ...
    data() {
      ...
      // var checkChildDemand = (rule, value, callback) => {
      //   checkChildDemandOnUpdate(this.ruleForm).then(res => {
      //     if (res.data != null && value == res.data.childDemandName) {
      //       callback(new Error("子需求编号已存在"));
      //     } else {
      //       callback();
      ...
      return {
        ...
        rules: {
          ...
          childDemandName:[
            ...
            // {validator:checkChildDemand,trigger: "blur"}
          ],
          parentDemandName:[
    ...
    methods: {
      ...
      getUploadPath(){
        this.uploadPath = process.env.BASE_API + "businessZipFile/update/"+this.ruleForm.programmeId;
      ...
      submitForm(formName) {
        checkChildDemandOnUpdate(this.ruleForm).then(res => {
          if (res.data != null && this.ruleForm.childDemandName == res.data.childDemandName) {
            this.$message({
              type: "error",
              message: "子需求编号已存在"
            })
          }else{
            this.ruleForm.zipFileList = this.fileList
            this.$refs[formName].validate(valid => {
              if (valid) {
                editProgramme(this.ruleForm).then(res => {
                  console.log("success");
    ...
    created() {
      ...
      getListById(this.ruleForm).then(res => {
        this.fileList=res.data;
      });
      this.$set(this.ruleForm, 'childProductIdArray', this.ruleForm.childProductId.split(","));
      this.getUploadPath();
    }
  };
</script>
